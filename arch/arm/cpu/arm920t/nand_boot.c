...
inline void led_on_1(void) {rGPBDAT &= ~(1<<5);}
inline void led_on_2(void) {rGPBDAT &= ~(1<<6);}
inline void led_on_3(void) {rGPBDAT &= ~(1<<7);}
inline void led_on_4(void) {rGPBDAT &= ~(1<<8);}

static void spl_nand_read_buf(unsigned char *buf, int len)
{
	int i;
...
static int spl_nand_dev_ready(void)
{
	return !!(rNFSTAT & 0x01);
}

static void spl_nand_select_chip(int chip)
{
	switch (chip) {
	case -1:
...
static void spl_nand_hwcontrol(int cmd, unsigned int ctrl)
{
	if (ctrl & NAND_NCE)
		spl_nand_select_chip(0);
	else
		spl_nand_select_chip(-1);
...
static void spl_nand_enable_hwecc(void)
{
...
static int spl_nand_calculate_ecc(unsigned char *ecc_code)
{
...
static int spl_nand_correct_data(unsigned char *dat,
 ...
 */
static int spl_nand_command(int block, int page, int offs, unsigned char cmd)
{
	int page_addr = page + block * CONFIG_SYS_NAND_PAGE_COUNT;

	while (!spl_nand_dev_ready());
	...
	spl_nand_hwcontrol(cmd, NAND_CTRL_CLE);
	/* Set ALE and clear CLE to start address cycle */
	/* Column address */
	spl_nand_hwcontrol(offs & 0xff, NAND_CTRL_ALE); /* A[7:0] */
	spl_nand_hwcontrol((offs >> 8) & 0xff, NAND_CTRL_ALE); /* A[11:9] */
	/* Row address */
	spl_nand_hwcontrol((page_addr & 0xff), NAND_CTRL_ALE); /* A[19:12] */
	spl_nand_hwcontrol(((page_addr >> 8) & 0xff),
		       NAND_CTRL_ALE); /* A[27:20] */
	spl_nand_hwcontrol((page_addr >> 16) & 0x0f,
		       NAND_CTRL_ALE); /* A[31:28] */

	/* Latch in address */
	spl_nand_hwcontrol(NAND_CMD_READSTART, NAND_CTRL_CLE);
	spl_nand_hwcontrol(NAND_CMD_NONE, NAND_NCE);
	/*
...
{
	spl_nand_command(block, 0, CONFIG_SYS_NAND_BAD_BLOCK_POS, NAND_CMD_READOOB);
...
static int spl_nand_read_page(int block, int page, unsigned char *dst)
{
	const static int ecc_pos[] = {40, 41, 42, 43};
	...
	spl_nand_command(block, page, 0, NAND_CMD_READ0);

	spl_nand_enable_hwecc();
	spl_nand_read_buf(p, eccsize);
	spl_nand_calculate_ecc(&ecc_calc[0]);

	spl_nand_read_buf(oob_data, 64);

	/* Pick the ECC bytes out of the oob data */
	for (i = 0; i < eccbytes; i++)
		ecc_code[i] = oob_data[ecc_pos[i]];
...
static int spl_nand_load(unsigned int offs, unsigned int uboot_size, unsigned char *dst)
{
...
void nand_boot(void)
{
	 ...
	 */
	spl_nand_load(0, CONFIG_SYS_NAND_U_BOOT_SIZE,
